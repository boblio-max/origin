# Origin Programming Language · v1.7.15

[![Status](https://img.shields.io/badge/Status-Stable-success?style=flat-square)](https://docs-origin.onrender.com)
[![Version](https://img.shields.io/badge/Version-v1.7.15-blue?style=flat-square)](https://docs-origin.onrender.com)
...
> **July 2026 re-issue:** The standalone `origin.exe` has been rebuilt from the v1.7.15 reference sources with the flat-layout imports normalized, and `secure_install.ps1` now hard-codes the SHA-256 of the new binary. The v1.7.15 release adds typed parameters and typed class fields (`def add(a: int, b: int)`, `class Point(x: int, y: int)`), typed loop variables and condition casts, type-aware casts on builtin arguments (`len(x: str)`, `range(0: int, 10: int, 2: int)`), auto-inference for untyped `let`/`const` declarations with a warning nudge, and a `range` step argument. The bytecode VM migration continues on the [`origin-dev`](https://github.com/boblio-max/origin-dev) branch and will ship as v1.8.
...
1. **Download**: [Origin v1.7.15 Stable](https://docs-origin.onrender.com/download.html)
